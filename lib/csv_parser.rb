...
module CSVParser
  ...
  def formatting_grade_level(unformatted_grade_level)
end
    ...
    all_data = []
     ...
     i.times do
      ...
      enrollment_by_year = group_names(all_parsed_data)
        unformatted_grade_level = file_tree.values[0].keys[path_counter]
        formatted_grade_level = unformatted_grade_level.id2name.insert(12, '_participation')
        grade_level = formatted_grade_level.intern
