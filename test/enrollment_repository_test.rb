...
class EnrollmentRepositoryTest < Minitest::Test
  ...
  def test_that_it_finds_both_instances
    enrollment1 = Enrollment.new({:name => "ACADEMY 20"})
    enrollment2 = Enrollment.new({:name => "Pizza academy 30"})
    enrollment3 = Enrollment.new({:name => "Pizza academy 30"})
    er          = EnrollmentRepository.new([enrollment1, enrollment2, enrollment3])
    assert_equal [enrollment2, enrollment3], er.find_by_name("Pizza academy 30")
  end
  ...
  def test_loading_enrollments
    ...
    er.load_data({
      ...
      assert_equal "ACADEMY 20", enrollment[0].name
    end
end
